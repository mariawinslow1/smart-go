...
export default function FreeAudit() {
  ...
  return <>
    ...
    <section className="audit-page"><div className="container audit-grid"><div className="audit-copy"><div className="eyebrow">Бесплатный первичный разбор</div><h1>Оставьте ссылку — получите 3 наблюдения по отзывам</h1><p>Я покажу повторяющийся сигнал, объясню, чем он важен, и предложу первое действие. Каждый вывод можно будет проверить по публичным отзывам.</p><div className="audit-price"><span>Стоимость</span><strong>{siteConfig.auditPrice}</strong></div><p className="audit-promise">Сначала вы увидите результат. Затем сами решите, нужен ли полный анализ.</p><a className="btn btn-primary audit-jump" href="#audit-form">Оставить ссылку</a><div className="audit-benefits">{['Конкретная ситуация, а не общая тема', 'Примеры, на которых основан вывод', 'Первое действие для проверки'].map(x => <div key={x}><Check /><span>{x}</span></div>)}</div><div className="audit-note"><strong>Доступы не нужны</strong><p>Достаточно ссылки на сайт, карточку организации, товар или страницу с отзывами.</p></div></div>
